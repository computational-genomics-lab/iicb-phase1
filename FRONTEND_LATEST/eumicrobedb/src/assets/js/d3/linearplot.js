...
var currentTrackStart;
var currentTrackEnd;
var fromTop = 0;

var currentLink = new URLSearchParams( window.location.href );
...
function genomeTrack(layout, tracks) {
	...
	this.trackLabels = this.chart.append("g").attr("id", "trackLblContainer");
	...
	this.tip = d3.tip()
		...
		.offset([-10, 0])
		.html(function (d) {
			document.querySelector(".genome-feature-label").value = d.name;
			return d.name.length ? `<strong>Name:</strong> <span style='color:white'>${d.name}</span>` : '';
		});
	...
	for (var i = 0; i < this.tracks.length; i++) {
		...
		switch (this.tracks[i].trackType) {
			...
			case "stranded":
				...
				// TODO: Add 1px rectangle box
				this.tracks[i].items.push( getBlankRect() );
...
genomeTrack.prototype.displayStranded = function (track, i) {
		...
		.each(function (d) {
			...
			if (typeof d.feature === 'undefined' || d.feature == "gene") {
				d3.select(this)
					...
					.attr("width", function (d) { return d.width; })
					.attr("height", function (d) { return linearTrackDefaults.track_height;
					...
					.attr("style", function(d){
						return "fill:" + ( d.color ? d.color : linearTrackDefaults.color );
					})
					.on("click", function (d, i) {

						if( d.link ) {
							let searchParams = new URLSearchParams(d.link);
							window.open( window.location.origin + "/browser-detail?" + searchParams, '_blank' );
						}
	...
	// Add track label
	if( ! document.getElementById( getTrackId( track.trackName ) ) ) {
		this.trackLabels.append("text")
		.text( `\uf06e ${track.trackName}` )
		.attr({
			dx: "0px",
			dy: `${fromTop + 7}px`,
			id: getTrackId( track.trackName ),
			style: "font-family:'Font Awesome 5 Free'; font-size: 11; cursor: pointer"
		})
		.on("click", function() {
			let elem = document.querySelector(`[class="${ track.trackName }"]`);
			let visible = elem.style.display;
			if( visible == "none" ) {
				elem.style.display = "block";
				this.innerHTML = `\uf06e ${ track.trackName }`;
		...
		.each(function (d) {
			...
			if (typeof d.feature === 'undefined' || d.feature == "gene") {
				...
				d3.select(this).append("rect")
					...
					.attr("class", function (d) { return track.trackName + ' ' + ((d.width > 5) ? (track.trackName + '_zoomed') : '') + ' ' + ('undefined' !== typeof d.extraclass ? d.extraclass : ''); })
					.attr("width", function (d) { return d.width < 5 ? 5 : d.width; })
					.attr("height", function (d) { return linearTrackDefaults.track_height;
						if (track.trackFeatures == 'complex') {
							var scale_factor = 0.77;
						} else {
					...
					.on("click", function (d, i) {
						...
						if( d.link ) {
							...
							if( label == "Non Coding region" ) {
								label = currentLink.searchParams.get("label");
							}
							let gotoUrl = window.location.origin + "/non-coding?" + searchParams + "&label=" + encodeURI( label );
...
genomeTrack.prototype.redraw = function () {
	var currentLink = new URLSearchParams( window.location.href );
	document.querySelector(".genome-position").value = `${ currentLink.get("scaffold") }:${ this.visStart.toFixed(0) }-${ this.visEnd.toFixed(0) }`;
	document.querySelector(".genome-location-label").innerText = `${ this.visStart.toFixed(0) }-${ this.visEnd.toFixed(0) }`;
...
function getTrackId( trackName ) {
	return "track_" + trackName.replace(/ /g, "").toLowerCase();
}


function getBlankRect() {
	return {
		"end": 1,
		"link": {},
		"id": 0,
